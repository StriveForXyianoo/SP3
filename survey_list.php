...
$link = "http://SP3/answer.php?survey_id=";
?>
<div class="col-lg-12">
		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT * FROM survey_set order by date(start_date) desc,date(end_date) desc ");
					...
					<tr>
						...
						$data = $link.$row['id'];
						echo '<img src="'.(new QRCode)->render($data).'" alt="QR Code" style="height:75px;width:75x;" />';

						?></td>
						<td><b><?php echo ucwords($row['title']) ?></b></td>
						<td><b class="truncate"><?php echo $row['description'] ?></b></td>
						<td><b><?php echo date("M d, Y-H:i A", strtotime($row['start_date'])) ?></b></td>
						<td><b><?php echo date("M d, Y-H:i A", strtotime($row['end_date'])) ?></b></td>
